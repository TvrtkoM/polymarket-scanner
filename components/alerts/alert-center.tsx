  ...
  return (
    <DropdownMenu onOpenChange={(open) => { if (open) markAllRead() }}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" aria-label="Alert center" className="relative">
      ...
      <DropdownMenuContent align="end" className="w-80 max-h-[480px] overflow-y-auto">
        ...
        <DropdownMenuSeparator />
        {recent.length === 0 ? (
          <p className="px-3 py-4 text-center text-sm text-muted-foreground">No alerts yet.</p>
        ) : (
          recent.map((alert) => (
            <Link
              key={alert.id}
            ...
            >
              <p className="text-xs font-medium line-clamp-1">{alert.question}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{alert.message}</p>
              <p className="text-[10px] text-muted-foreground/60 mt-1">{formatDate(alert.firedAt)}</p>
            </Link>
          ))
        )}
      </DropdownMenuContent>
